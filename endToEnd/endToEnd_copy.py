...
if __name__ == '__main__':
    ...
    if loadCheckpoint:
        model_CKPT = torch.load('./finalEpochWeight_firstweight.pth')
    ...
    for fold in foldList:
        ...
        for epoch in range(50):
            ...
            if os.path.exists('./finalEpochWeight_'+str(epoch-1)+'.pth'):
                os.remove('./finalEpochWeight_'+str(epoch-1)+'.pth')
            torch.save({'epoch': epoch + 1,
                        'state_dict': endToEndModel.state_dict(),
                        'optim_dict' : optimizer.state_dict()},
                        'checkpoint/finalEpochWeight_epoch'+str(epoch)+'_fold'+str(fold)+'.pth')
