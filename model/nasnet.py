...
class SeperableBranch(nn.Module):
    ...
    def __init__(self, input_channels, output_channels, kernel_size, **kwargs):
        ...
        self.block1 = nn.Sequential(
            nn.ReLU(),
            SeperableConv3d(input_channels, output_channels, kernel_size, **kwargs),
            nn.BatchNorm3d(output_channels)
        )

        self.block2 = nn.Sequential(
            nn.ReLU(),
            SeperableConv3d(output_channels, output_channels, kernel_size, stride=1, padding=int(kernel_size / 2)),
            nn.BatchNorm3d(output_channels)
        )
...
class Fit(nn.Module):
    ...
    def __init__(self, prev_filters, filters):
        ...
        self.p1 = nn.Sequential(
            nn.AvgPool3d(1, stride=2),
            nn.Conv3d(prev_filters, int(filters / 2), 1)
        )

        #make sure there is no information loss
        self.p2 = nn.Sequential(
            nn.ConstantPad3d((0, 1, 0, 1), 0),
            nn.ConstantPad3d((-1, 0, -1, 0), 0),   #cropping
            nn.AvgPool3d(1, stride=2),
            nn.Conv3d(prev_filters, int(filters / 2), 1)
        )
        ...
        self.dim_reduce = nn.Sequential(
            nn.ReLU(),
            nn.Conv3d(prev_filters, filters, 1),
...
class NormalCell(nn.Module):
    ...
    def __init__(self, x_in, prev_in, output_channels):
        ...
        self.dem_reduce = nn.Sequential(
            nn.ReLU(),
            nn.Conv3d(x_in, output_channels, 1, bias=False),
            nn.BatchNorm3d(output_channels)
        )
        ...
        self.block3_left = nn.AvgPool3d(3, stride=1, padding=1)
        self.block3_right = nn.Sequential()

        self.block4_left = nn.AvgPool3d(3, stride=1, padding=1)
        self.block4_right = nn.AvgPool3d(3, stride=1, padding=1)
...
class ReductionCell(nn.Module):
    ...
    def __init__(self, x_in, prev_in, output_channels):
        ...
        self.dim_reduce = nn.Sequential(
            nn.ReLU(),
            nn.Conv3d(x_in, output_channels, 1),
            nn.BatchNorm3d(output_channels)
        )
    ...
    def forward(self, x):
        ...
        a = self.layer2block2_left(layer1block1)    #4,88,4,64,64
        b = self.layer2block2_right(layer1block2)   #4,88,4,64,64
...
class NasNetA(nn.Module):
    ...
    def __init__(self, repeat_cell_num, reduction_num, filters, stemfilter, class_num=2):
        ...
        self.stem = nn.Sequential(
            nn.Conv3d(1, stemfilter, 3, padding=1, bias=False),
            nn.BatchNorm3d(stemfilter)
        )
        ...
        self.fc1 = nn.Linear(self.filters * 6, 512)
