...
class LinearBottleneck(nn.Module):
    def __init__(self, inplanes, outplanes, stride=1, t=6, activation=nn.ReLU6):
        ...
        self.conv1 = nn.Conv3d(inplanes, inplanes * t, kernel_size=1, bias=False)
        self.bn1 = nn.BatchNorm3d(inplanes * t)
        self.conv2 = nn.Conv3d(inplanes * t, inplanes * t, kernel_size=3, stride=stride, padding=1, bias=False,
                               groups=inplanes * t)
        self.bn2 = nn.BatchNorm3d(inplanes * t)
        self.conv3 = nn.Conv3d(inplanes * t, outplanes, kernel_size=1, bias=False)
...
class MobileNet2(nn.Module):
    ...
    def __init__(self, scale=1.0, input_size=224, t=6, in_channels=1, num_classes=2, activation=nn.ReLU6):
        """
        ...
        self.conv1 = nn.Conv3d(in_channels, self.c[0], kernel_size=3, bias=False, stride=self.s[0], padding=1)
        self.bn1 = nn.BatchNorm3d(self.c[0])
        ...
        self.conv_last = nn.Conv3d(self.c[-1], self.last_conv_out_ch, kernel_size=1, bias=False)
        self.bn_last = nn.BatchNorm3d(self.last_conv_out_ch)
        self.avgpool = nn.AdaptiveAvgPool3d(1)
        self.dropout = nn.Dropout(p=0.2, inplace=True)  # confirmed by paper authors
        self.fc1 = nn.Linear(self.last_conv_out_ch, 512)
