...
class SeperableConv3d(nn.Module):
    ...
    def __init__(self, input_channels, output_channels, kernel_size, **kwargs):
        ...
        self.pointwise = nn.Conv3d(input_channels, output_channels, 1, bias=False)
...
class EntryFlow(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv3_residual = nn.Sequential(
            SeperableConv3d(64, 128, 3, padding=1),
            nn.BatchNorm3d(128),
            nn.ReLU(inplace=True),
            SeperableConv3d(128, 128, 3, padding=1),
            nn.BatchNorm3d(128),
            nn.MaxPool3d(3, stride=2, padding=1),
        )

        self.conv3_shortcut = nn.Sequential(
            nn.Conv3d(64, 128, 1, stride=2),
            nn.BatchNorm3d(128),
        )

        self.conv4_residual = nn.Sequential(
            nn.ReLU(inplace=True),
            SeperableConv3d(128, 256, 3, padding=1),
            nn.BatchNorm3d(256),
            nn.ReLU(inplace=True),
            SeperableConv3d(256, 256, 3, padding=1),
            nn.BatchNorm3d(256),
            nn.MaxPool3d(3, stride=2, padding=1)
        )

        self.conv4_shortcut = nn.Sequential(
            nn.Conv3d(128, 256, 1, stride=2),
            nn.BatchNorm3d(256),
        )
        ...
        self.conv5_residual = nn.Sequential(
            nn.ReLU(inplace=True),
            SeperableConv3d(256, 728, 3, padding=1),
            nn.BatchNorm3d(728),
            nn.ReLU(inplace=True),
            SeperableConv3d(728, 728, 3, padding=1),
            nn.BatchNorm3d(728),
            nn.MaxPool3d(3, 1, padding=1)
        )
...
class ExitFLow(nn.Module):
    ...
    def __init__(self):
        ...
        self.residual = nn.Sequential(
            ...
            nn.BatchNorm3d(728),
            nn.ReLU(),
            SeperableConv3d(728, 1024, 3, padding=1),
            nn.BatchNorm3d(1024),
            nn.MaxPool3d(3, stride=2, padding=1)
        )

        self.shortcut = nn.Sequential(
            nn.Conv3d(728, 1024, 1, stride=2),
            nn.BatchNorm3d(1024)
        )

        self.conv = nn.Sequential(
            SeperableConv3d(1024, 1536, 3, padding=1),
            nn.BatchNorm3d(1536),
            nn.ReLU(inplace=True),
            SeperableConv3d(1536, 2048, 3, padding=1),
            nn.BatchNorm3d(2048),
            nn.ReLU(inplace=True)
        )
