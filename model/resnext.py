...
class ResNextBottleNeckC(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride):
        ...
        self.split_transforms = nn.Sequential(
            nn.Conv3d(in_channels, C * D, kernel_size=1, groups=C, bias=False),
            nn.BatchNorm3d(C * D),
            nn.ReLU(inplace=True),
            nn.Conv3d(C * D, C * D, kernel_size=3, stride=stride, groups=C, padding=1, bias=False),
            nn.BatchNorm3d(C * D),
            nn.ReLU(inplace=True),
        ...
        if stride != 1 or in_channels != out_channels * 4:
            self.shortcut = nn.Sequential(
                nn.Conv3d(in_channels, out_channels * 4, stride=stride, kernel_size=1, bias=False),
                nn.BatchNorm3d(out_channels * 4)
            )
...
class ResNext(nn.Module):
    ...
    def __init__(self, block, num_blocks, class_names=2):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv3d(1, 64, 3, stride=1, padding=1, bias=False),
            nn.BatchNorm3d(64),
            nn.ReLU(inplace=True)
        )
        ...
        self.fc2 = nn.Linear(512, class_names)
