...
class ResnetInit(nn.Module):
    def __init__(self, in_channel, out_channel, stride):
        ...
        #(parameters W l,r→r )
        self.residual_stream_conv = nn.Conv3d(in_channel, out_channel, 3, padding=1, stride=stride)

        #"""and a transient stream, t, which is a standard convolutional layer
        #(W l,t→t )."""
        self.transient_stream_conv = nn.Conv3d(in_channel, out_channel, 3, padding=1, stride=stride)

        #"""Two additional sets of convolutional filters in each block (W l,r→t , W l,t→r )
        #also transfer information across streams."""
        self.residual_stream_conv_across = nn.Conv3d(in_channel, out_channel, 3, padding=1, stride=stride)
        ...
        self.transient_stream_conv_across = nn.Conv3d(in_channel, out_channel, 3, padding=1, stride=stride)
...
class RiRBlock(nn.Module):
    def __init__(self, in_channel, out_channel, layer_num, stride, layer=ResnetInit):
        ...
        #    self.short_cut = nn.Conv3d(in_channel, out_channel, kernel_size=1, stride=stride)
...
class ResnetInResneet(nn.Module):
    def __init__(self, num_classes=2):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv3d(384, num_classes, kernel_size=(1,3,3), stride=(1,2,2)), #without this convolution, loss will soon be nan
