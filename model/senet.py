...
class BasicResidualSEBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride, r=16):
        ...
        self.residual = nn.Sequential(
            ...
            nn.Conv3d(out_channels, out_channels * self.expansion, 3, padding=1),
    ...
    def forward(self, x):
        shortcut = self.shortcut(x) #[1, 64, 8, 128, 128]
        residual = self.residual(x)#[1, 64, 8, 128, 128]

        squeeze = self.squeeze(residual)
        squeeze = squeeze.view(squeeze.size(0), -1)
        excitation = self.excitation(squeeze)
        excitation = excitation.view(residual.size(0), residual.size(1), 1, 1, 1) #[1, 64, 1, 1, 1]
        a= excitation.expand_as(residual)
...
class BottleneckResidualSEBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride, r=16):
        ...
        self.residual = nn.Sequential(
            ...
            nn.Conv3d(out_channels, out_channels, 3, stride=stride, padding=1),
            nn.BatchNorm3d(out_channels),
            nn.ReLU(inplace=True),
